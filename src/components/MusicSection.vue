...
const playing = reactive(tracks.map(() => false))
const times = reactive(tracks.map(() => '0:00 / 0:00'))
const canvasEls = reactive(tracks.map(() => null))

const players = tracks.map(() => null)
...
function buildPlayer(i) {
  ...
  p.drawLoop = function drawLoop() {
    ...
    const bars = 28
    const gap = 2
    const bw = Math.max(1, Math.floor(w / bars) - gap)
    ...
    if (!p.playing) {
      ...
      for (let j = 0; j < bars; j++) {
        c.fillStyle = j % 2 === 0 ? '#3B82F6' : '#06B6D4'
        c.fillRect(j * (bw + gap), h - 2, bw, 2)
      }
      return
    }
